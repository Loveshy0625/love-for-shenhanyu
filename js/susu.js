...
(function () {
    ...
    window.vm = new Vue({
        ...
        computed: {
            hasOptions: function() {
                var has = false;
                if (this.isFinished) has = true;
                else if (this.isTyping) has = false;
                else if (this.lastDialog && this.lastDialog.responses && this.lastDialog.responses.length > 0) {
                    has = true;
                }
                console.log('hasOptions 计算:', {
                    isFinished: this.isFinished,
                    isTyping: this.isTyping,
                    lastDialog: this.lastDialog,
                    responses: this.lastDialog ? this.lastDialog.responses : null,
                    hasOptions: has
                });
                return has;
            }
        ...
        mounted: function () {
            var _this = this;
            console.log('苏苏页面加载，读取对话数据...');
            
            fetch('./assets/susu-dialog.json')
                ...
                .then(function(data) {
                    console.log('苏苏对话数据加载成功:', data);
                    _this.dialogs = data;
                    _this.isInitialized = true;
                    console.log('苏苏页面初始化完成，开始对话');
                    // 自动开始对话
                    setTimeout(function() {
                        _this.appendDialog('susu001');
                    }, 500);
                })
                .catch(function(error) {
                    console.error('加载苏苏对话失败:', error);
        ...
        methods: {
            appendDialog: function (id) {
                ...
                var messageArray = dialog.details && dialog.details.length > 0 ? dialog.details[0] : [];
            ...
            togglePrompt: function (toShow) {
                console.log('togglePrompt called:', toShow, 'isTyping:', this.isTyping, 'hasPrompt:', this.hasPrompt);
                if (this.isTyping && toShow) {
                    console.log('阻止显示：正在输入中');
                    return;
                }
                this.hasPrompt = toShow;
                console.log('hasPrompt 设置为:', this.hasPrompt);
            },
